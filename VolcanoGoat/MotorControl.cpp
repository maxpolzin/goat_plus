...
MotorControl::MotorControl(int leftChannel, int rightChannel, int winchChannel)
  : PWM_LEFT_CHANNEL(leftChannel), PWM_RIGHT_CHANNEL(rightChannel), PWM_WINCH_CHANNEL(winchChannel) {}


void MotorControl::begin(int leftPin, int rightPin, int winchPin) {
...
void MotorControl::update(int forwardCommand, int steeringCommand, int winchCommand) {
  ...
  ledcAnalogWrite(PWM_WINCH_CHANNEL, winchPWM);
}
